...
public class TaskDAO  extends AbstractJpaDao<Long, Task> {
  ...
  public List<Task> findByDescription(String description){
    return entityManager.createQuery(
            "SELECT e FROM "+ clazz.getSimpleName() +" e WHERE e.description LIKE :description",clazz)
  ...
  public List<Task> findByStatusAndPriority(TaskPriority priority, TaskStatus status){
    return entityManager.createQuery(
            "SELECT e FROM "+ clazz.getSimpleName() +" e WHERE e.status = :status AND e.priority = :priority",clazz)
            .setParameter("priority", priority)
            .setParameter("status", status)
            .getResultList();
  }
  ...
  public List<Task> findByTypeAndPriority(TaskPriority priority, TaskType type){
    return entityManager.createQuery(
            "SELECT e FROM "+ clazz.getSimpleName() +" e WHERE e.type = :type AND e.priority = :priority",clazz)
        .setParameter("priority", priority)
        .setParameter("type", type)
        .getResultList();
  }

  public List<Task> findByAssignee(Assignee assigned){
    return entityManager.createQuery(
            "SELECT e FROM "+ clazz.getSimpleName() +" e WHERE e.assigned = :assigned",clazz)
